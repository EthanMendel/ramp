...
/** return angle that will form a straight line from position a to position b. a and b are [x, y] vectors */
...
/** return distance between orientations a1 and a2; distance is in the range [-PI, PI]. */
const double Utility::findDistanceBetweenAngles(const double a1, const double a2) const
{
    double result;
    double difference = a2 - a1;

    // difference > pi, result should be in [-PI,0] range
    if (difference > PI){
        difference = fmodf(difference, PI);
        result = difference - PI;
    }
    // difference < -pi, the should be in [0,PI] range
