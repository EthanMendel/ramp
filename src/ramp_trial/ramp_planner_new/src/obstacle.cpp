...
visualization_msgs::Marker Obstacle::getMarker(std::string global_frame, int id){
    ...
    if(numBounds_ == 2){
        ...
        // set radii
        marker.scale.x = 2*bounds_.at(1).at(0);
        marker.scale.y = 2*bounds_.at(1).at(1);
        marker.scale.z = 2*bounds_.at(1).at(2);
    ...
    //     marker.scale.z = 0.1;
        ...
        std::cout<<"MORE THAN 2 BOUNDING POINTS FOR OBSTACLE"<<std::endl;
    }
