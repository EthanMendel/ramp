...
void initSwappedStartGoal(const std::vector<geometry_msgs::Point> points){
  double pastY;
...
void getTrajectory(ramp_planner_new::TrajectoryRequest msg){
  std::cout<<"getting "<<msg.type<<" trajectory"<<std::endl;
    if(msg.swapped){
      ...
      if(msg.newTrajPoints.size() > 0){
        initSwappedStartGoal(msg.newTrajPoints);
      }else{
    ...
    for(unsigned int i=0;i<pathPoints.markers.size();i++){
        if(!replaced){//before finding marker to 'replace'
            if(pathPoints.markers.at(i) == m){//if marker marker
                std::cout<<"found marker at i="<<i<<std::endl;
...
void bezify(const ramp_planner_new::BezifyRequest& br){
  ...
  if(br.markers.size() == 3){
    ...
    float d;
    for(d=.2;d<=D;d+=.1){//for all distences (incrimented as 10ths)
        cp1.x = p1.x - d*u1[0];
        cp1.y = p1.y - d*u1[1];
        cp2.x = p1.x + d*u2[0];
        cp2.y = p1.y + d*u2[1];
...
int main(int argc, char** argv) {
  ...
  pub_path_points = handle.advertise<ramp_planner_new::PathPoints>("single_path_points_channel",10);
